...
class UpdateMedicineUseCase:
    ...
    def execute(self, input_data: UpdateMedicineInput) -> UpdateMedicineOutput:
        ...
        try:
            # Ne met à jour que les champs réellement fournis (évite d'écraser
            # avec None et évite le crash sur None.strip())
            if input_data.nom is not None:
                medicament.nom = input_data.nom.strip()
            if input_data.prix is not None:
                medicament.prix = input_data.prix
            if input_data.quantite is not None:
                medicament.quantite = input_data.quantite
            if input_data.date_expiration is not None:
